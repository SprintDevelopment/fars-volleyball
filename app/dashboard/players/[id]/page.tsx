...
import { z } from "zod";
import { getPlayer, updatePlayer } from "@/lib/playersService";
...
import { PlayerUpdate, UpdatePlayerSchema } from "@/lib/schemas/player-schema";

export default function EditPlayerPage() {
  const { id } = useParams() as {id: string}; // گرفتن ID از URL
  ...
  useEffect(() => {
    async function fetchPlayer() {
      try {
        ...
        form.reset(player); // مقداردهی فرم با اطلاعات بازیکن
      } catch (error) {
  ...
  const onSubmit = async (data: PlayerUpdate) => {
    try {
